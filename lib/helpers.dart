...
class EmailService {
  ...
  Future<void> sendInvoiceEmail(String receiverEmail, File pdfFile) async {
    ...
    final message = Message()
      ..from = Address(username, 'Dental Care')
      ..recipients.add(receiverEmail)
      ..subject = 'Payment Success to Dental Care'
      ..html = '''
      <div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
        <div style="margin:50px auto;width:70%;padding:20px 0">
          <div style="border-bottom:1px solid #eee">
          ...
          <p style="font-size:1.1em">Hi,</p>
          <p>Your payment to Dental Care was successful. Please find your payment receipt attached as a PDF.</p>
          <p style="font-size:0.9em;">Regards,<br />Dental Care</p>
          <hr style="border:none;border-top:1px solid #eee" />
          <div style="float:right;padding:8px 0;color:#aaa;font-size:0.8em;line-height:1;font-weight:300">
            <p>Dental Care</p>
    ...
    try {
      final sendReport = await send(message, smtpServer);
      print('Invoice Email sent: ' + sendReport.toString());
    } on MailerException catch (e) {
  ...
  Future<void> sendPrescriptionEmail(
    ...
    final message = Message()
      ..from = Address(username, 'Dental Care')
      ..recipients.add(receiverEmail)
      ..subject = 'Your Prescription from Dental Care'
      ..html = '''
      <div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
        <div style="margin:50px auto;width:70%;padding:20px 0">
          <div style="border-bottom:1px solid #eee">
    ...
    try {
      final sendReport = await send(message, smtpServer);
      print('Prescription Email sent: ' + sendReport.toString());
    } on MailerException catch (e) {
